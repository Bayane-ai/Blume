...
jest.mock("../lib/sports/basketball/provider", () => ({ getBasketballApiKey: () => "basket-key" }));
jest.mock("../lib/sports/tennis/provider", () => ({ getTennisApiKey: () => "tennis-key" }));
...
test("GET : renvoie le taux de réussite et le statut des combinés demandés (ids séparés par des virgules)", async () => {
  ...
  expect(maintainAndGetComboStats).toHaveBeenCalledWith(
    ["a", "b", "c"],
    expect.objectContaining({ basketballApiKey: "basket-key", tennisApiKey: "tennis-key" })
  );
