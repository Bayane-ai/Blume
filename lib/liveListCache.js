import { LIVE_STATUSES as LIVE_STATUSES_LIST, LIVE_STATUS_QUERY } from "./liveStatuses";
...
// Statuts correspondant à un match réellement en cours, y compris prolongations et
// tirs au but (voir lib/liveStatuses.js — plus jamais une copie locale qui pourrait
// diverger et exclure un match pourtant en direct).
const LIVE_STATUS_FILTER = LIVE_STATUS_QUERY;
const LIVE_STATUSES = new Set([...LIVE_STATUSES_LIST, "LIVE"]);
...
export async function getLiveMatchesList(token) {
  ...
  inFlight = (async () => {
    try {
      ...
      if (!r.ok) {
        // Lit le corps de la réponse d'erreur (football-data.org renvoie en général un
        // vrai message JSON, ex: {"message":"Invalid credentials"}) — voir PROMPT :
        // "message d'erreur explicite... avec le motif technique", jamais juste un code.
        const errorBody = await r.json().catch(() => null);
        console.error(`Erreur football-data.org (matchs en direct) : HTTP ${r.status}`, errorBody);
        ...
        cachedResult = { errorStatus: r.status, errorMessage: errorBody?.message || null };
        cachedAt = Date.now();
        return cachedResult;
      }
      ...
      console.error("Erreur réseau football-data.org (matchs en direct) :", e.message);
      if (cachedResult && cachedResult.matches) return cachedResult;
      cachedResult = { errorStatus: null, networkError: e.message };
      cachedAt = Date.now();
      return cachedResult;
    } finally {
      inFlight = null;
