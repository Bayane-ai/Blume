...
import { getMatchScore as getTennisMatchScore } from "./sports/tennis/provider";
import { mapMatchToLiveState } from "./sports/tennis/mapper";
...
async function fetchMatchState(leg, ctx) {
  ...
  if (sport === "tennis") {
    // Live Tennis API (voir lib/sports/tennis/provider.js) n'a pas de recherche par id
    // hors de l'endpoint de score détaillé lui-même — celui-ci suffit ici (statut,
    // sets gagnés, jeux par set), jamais besoin de la liste /matches?status=live pour
    // relire UN match déjà connu par son id.
    if (!ctx.tennisApiKey) return null;
    const realId = String(leg.matchId).replace(/^tn-/, "");
    const rawScore = await getTennisMatchScore(realId, ctx.tennisApiKey);
    if (!rawScore) return null;
    const liveState = mapMatchToLiveState({ id: realId, status: rawScore?.status }, rawScore);
    return {
      status: liveState.status,
      score: liveState.score?.fullTime || null,
      games: tennisGameTotalsFromSets(liveState.sets),
      setsCount: liveState.sets?.length ?? null,
      raw: rawScore,
    };
  }
...
async function fetchRealStatsForLeg(leg, state, ctx) {
  ...
  // Tennis : plus aucune ligne ne nécessite un second appel réseau — totalGames vient
  // de `state.games` et totalSets de `state.setsCount` (voir fetchMatchState ci-dessus
  // et resolveRealValue plus bas), tous deux déjà connus sans appel supplémentaire.
  // Live Tennis API (plan gratuit, voir lib/sports/tennis/provider.js) ne fournit de
  // toute façon aucune statistique finale (aces, etc.) — cette ligne de marché n'est
  // donc plus proposée par lib/sports/tennis/livePronostic.js.
...
function resolveRealValue(sport, statKey, { score, state, realStats }) {
  ...
  if (sport === "tennis") {
    return {
      ...
      totalSets: state?.setsCount ?? null,
    }[statKey];
  }
  return null;
}
