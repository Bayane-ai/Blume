...
//
// BLOC 9 (multi-sport) — un combiné peut désormais mélanger football, basket et
// tennis : `match.sport` (renseigné par l'appelant, voir pages/combine-vision.js)
// identifie chaque match, et le pool `selectionCandidates` existe désormais aussi
// côté basket (lib/sports/basketball/pronosticModel.js) et tennis (lib/sports/tennis/
// pronosticModel.js), même forme que le football — ce fichier reste donc, comme
// avant, un simple assembleur qui ne recalcule rien lui-même, quel que soit le sport.
const LIVE_STATUSES = ["IN_PLAY", "PAUSED"];

// En dessous de ce seuil, l'issue 1X2 la plus probable n'est pas jugée "assez sûre"
// pour entrer dans un combiné (33,3 % est le neutre à 3 issues — football uniquement,
// seul sport de ce site avec un match nul possible).
const WINNER_MIN_CONFIDENCE = 45;
// Tous les autres marchés du pool (totaux de buts/points/jeux, tirs, cartons,
// corners, hors-jeu, fautes, touches, rebonds, passes décisives, aces...) sont des
// marchés Plus/Moins à 2 issues (neutre à 50 %) : même seuil, plus élevé que celui du
// 1X2 football, pour rester cohérent avec l'exigence "assez sûr" — BLOC 9 : c'est
// aussi ce seuil qui s'applique à l'issue du match basket/tennis (2 issues, jamais de
// nul), voir isEligible ci-dessous.
...
// son type de marché — voir WINNER_MIN_CONFIDENCE/MARKET_MIN_CONFIDENCE ci-dessus.
// BLOC 9 : "Issue du match" n'a un seuil abaissé (3 issues) qu'au football, seul sport
// avec un match nul possible — basket/tennis (2 issues, jamais de nul) utilisent le
// même seuil que les autres marchés Plus/Moins.
function isEligible(candidate, sport) {
  const threshold = candidate.marketLabel === "Issue du match" && sport === "football" ? WINNER_MIN_CONFIDENCE : MARKET_MIN_CONFIDENCE;
...
// BLOC 9 — cette vérification reste SPÉCIFIQUE au football (voir pickLegForMatch, qui
// ne l'appelle que pour ce sport) : basket/tennis n'ont pas cette notion de repli
// "estimation moyenne" — leurs profils (lib/sports/basketball/statProfiles.js, lib/
// sports/tennis/statProfiles.js) ont chacun leur propre repli à plusieurs niveaux,
// jamais un simple drapeau "fiable/pas fiable" comparable à celui du football.
...
// BLOC 9 — reste SPÉCIFIQUE au football pour l'instant : `minute` n'a pas le même
// sens pour le basket (chrono par quart-temps, voir lib/liveClockFormat.js) ni pour
// le tennis (score de jeu "40-30", pas une minute écoulée) — un seuil "tard dans le
// match" comparable nécessiterait sa propre logique par sport, pas encore construite
// ici ; les combinés basket/tennis en direct restent donc affichés sans ce signal
// d'alerte anticipée (jamais "compromis" à tort, jamais un signal inventé).
const COMPROMISED_MINUTE = 75;
function isLegCompromised(leg) {
  if (leg.sport && leg.sport !== "football") return false;
...
export function pickLegForMatch(match) {
  ...
  if (sport === "football" && !hasReliableStats(p)) return null;

  const homeName = p.home?.name || match.homeTeam?.name || "Domicile";
  const awayName = p.away?.name || match.awayTeam?.name || "Extérieur";

  const eligible = p.selectionCandidates.filter((c) => isEligible(c, sport));
  ...
  // (dernier recours par match, jamais un choix par défaut) — n'existe qu'au football
  // (CARTONS_LABELS ne contient aucun marché basket/tennis, ce filtre est donc sans
  // effet pour eux).
...
// BLOC 9 — comparateur d'ids STRING (jamais `a - b`, qui casse dès qu'un id n'est pas
// purement numérique — "bk-202"/"tn-45" pour le basket/tennis) : peu importe l'ordre
// exact obtenu, seule compte la DÉTERMINISME (les deux mêmes lignes produisent
// toujours la même clé, quel que soit l'ordre de tirage) — utilisé pour la clé de
// dédoublonnage des combinés (voir tryAddCombo/le tirage "En live" plus bas) et l'id
// du combiné lui-même.
function sortMatchIds(ids) {
  return [...ids].map(String).sort();
}

function buildCombo(legs, { riskLevel } = {}) {
  const confidence = combinedConfidence(legs);
  const sortedIds = sortMatchIds(legs.map((l) => l.matchId));
...
export function generateCombos(matches, { random = Math.random } = {}) {
  ...
  function tryAddCombo(riskLevel, { fromPool = pool } = {}) {
    ...
    const key = sortMatchIds(legs.map((l) => l.matchId)).join("-");
