...
// attendus, intensité offensive + équilibre des forces pour le reste). Ce fichier ne
// recalcule donc plus aucune statistique lui-même — il choisit juste, pour CE match,
// la sélection la plus fiable parmi ce pool, puis assemble les combinés. Tacles et
// homme du match restent absents : aucune source déjà connectée à ce site
// (football-data.org, API-Football) ne les fournit.
...
// aucune sélection n'atteint son seuil de confiance minimum (le match n'a alors aucune
// ligne "assez sûre" à proposer, et n'entre dans aucun combiné) — jamais une ligne
// inventée pour remplir un ticket.
...
export function generateCombos(matches, { random = Math.random } = {}) {
  ...
  // temps (2-3 lignes, comme les combinés pré-match) ; très risqué seulement de temps
  // en temps, avec la même rareté que le combiné pré-match très risqué ci-dessus (voir
  // PROMPT bloc 3 : "les opportunités live très risquées restent rares", pas
  // impossibles).
  ...
  if (livePool.length > 0) {
    ...
    const legCount = wantsRisky ? Math.min(RISKY_LEG_COUNT + 1, pool.length) : Math.min(3, pool.length);
