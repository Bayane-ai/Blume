...
function buildCombo(legs, { isLive = false, riskLevel } = {}) {
...
export function generateCombos(matches, { random = Math.random } = {}) {
  ...
  function tryAddCombo(riskLevel, { isLive = false, fromPool = pool } = {}) {
    ...
    combos.push(buildCombo(legs, { isLive, riskLevel }));
    return true;
  }
  ...
  if (livePool.length > 0) {
    ...
    if (legCount) {
      ...
      if (!usedComboKeys.has(key)) {
        usedComboKeys.add(key);
        combos.push(buildCombo(legs, { isLive: true, riskLevel: riskLevelForLegCount(legCount) }));
      }
    }
  }
