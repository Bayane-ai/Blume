...
// connexion se fait maintenant sans mot de passe, via Google OAuth ou un code reçu
// par email (voir pages/connexion.js) — dans les deux cas, c'est l'email qui est
// l'identifiant que le propriétaire choisit et reconnaît lui-même (contrairement à
// l'id Supabase, une valeur opaque qu'il ne voit jamais). "vérifié" n'est jamais une
// simple déclaration : Google confirme l'email automatiquement, et la connexion par
// code n'aboutit QUE si le code reçu sur CETTE boîte mail a été saisi avec succès —
// dans les deux cas, Supabase renseigne `email_confirmed_at` ; on l'exige
// explicitement ici pour ne jamais dépendre uniquement d'un champ `email` qui
// pourrait, sur un provider mal configuré, ne pas être vérifié.
//
...
export function isOwner(session) {
  ...
  // Email non vérifié (ne devrait jamais arriver via Google ou une connexion par code
