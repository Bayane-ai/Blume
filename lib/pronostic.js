...
function estimateMatchStats(lambdaHome, lambdaAway) {
  ...
  // jaunes) — une ligne "Moins de 0,5" serait presque toujours vraie et n'apporterait
  // aucune information, donc exprimé en probabilité qu'il y en ait au moins un plutôt
  // qu'en ligne Plus/Moins.
  ...
  const redCardProbability = round1(1 - poissonP(0, redCardExpected));
  ...
  return {
    ...
    cards: {
      ...
      redProbability: redCardProbability,
    },
    possession: { home: possessionHome, away: possessionAway },
    raw: { cornersTotal: cornersTotalRaw, shotsTotal: shotsTotalRaw, yellowCardsTotal: yellowCardsTotalRaw },
  };
}
...
// match, total de chaque équipe séparément (jamais mélangés), corners, tirs, cartons
// jaunes — chacune dérivée des vraies stats de CE match (buts attendus par équipe,
// intensité offensive), jamais une valeur fixe. Le Total (buts) accepte une marge de
// deux lignes quand l'issue est trop incertaine pour une seule (voir overUnderLine) ;
// les autres marchés restent sur une seule ligne, faute de distribution dédiée.
function buildMarkets({ totalHomeGoals, totalAwayGoals, extraStats }) {
  return {
    ...
    yellowCards: overUnderLine(extraStats.raw.yellowCardsTotal),
  };
}

const STATS_NOTE =
  "Corners, tirs, cartons et possession ne sont pas fournis par l'API (plan gratuit) : ce sont des estimations statistiques basées sur l'intensité offensive attendue de chaque équipe, pas une mesure réelle du match. Le carton rouge, événement rare, est exprimé en probabilité plutôt qu'en ligne Plus/Moins.";
