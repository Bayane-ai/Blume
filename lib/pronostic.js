...
// Confiance RÉELLE (marge statistique) d'une ligne Plus/Moins, selon le même modèle de
// Poisson que le reste du pronostic : la probabilité que l'issue choisie (`side`) se
// réalise pour CE match précis, à partir de l'estimation réelle `expected` — jamais une
// valeur arbitraire. Plus l'estimation réelle est loin de la ligne, plus cette
// confiance est élevée. Sert à la fois à l'affichage détaillé d'un match (marge
// utilisée par overUnderLine/riskLines pour décider d'élargir une ligne) et au pool de
// sélections "assez sûres" de Combiné Vision (voir buildSelectionCandidates plus bas).
function lineConfidence(expected, line, side) {
  const pOver = probabilityOverLine(line, Math.max(0, expected || 0));
  const confidence = side === "Plus" ? pOver : 1 - pOver;
  return round1(confidence); // round1 convertit une fraction (0..1) en pourcentage (voir plus haut)
}
...
export function overUnderLine(expected, { minLine = 0.5, withMargin = false } = {}) {
  ...
  const primaryConfidence = lineConfidence(value, primaryLine, side);
  const lines = [{ line: primaryLine, side, confidence: primaryConfidence }];

  if (withMargin && primaryConfidence < LINE_CONFIDENCE_THRESHOLD * 100) {
    const secondaryLine = side === "Plus" ? primaryLine + 1 : primaryLine - 1;
    if (secondaryLine >= minLine) {
      lines.push({ line: secondaryLine, side, confidence: lineConfidence(value, secondaryLine, side) });
    }
  }

  return { line: primaryLine, side, confidence: primaryConfidence, lines };
}
...
export function riskLines(expected, { minLine = 0.5 } = {}) {
  const value = Math.max(0, expected || 0);
  const primaryLine = Math.max(minLine, Math.floor(value) + 0.5);
  const side = value > primaryLine ? "Plus" : "Moins";
  ...
  return {
    safe: { line: safeLine, side, confidence: lineConfidence(value, safeLine, side) },
    risky: { line: riskyLine, side: riskySide, confidence: lineConfidence(value, riskyLine, riskySide) },
  };
}
...
export function rangeFromShare(point) {
...
export function matchProfileLabel(expectedTotal) {
...
export function favoriteSummary(probabilities, homeTeamName, awayTeamName) {
...
export function applyHeadToHead(lambdaHome, lambdaAway, h2h) {
...
// Part des tirs qui cadrent le but, en football professionnel (~33-37 % du total de
// tirs selon les championnats) — dérivée du même total de tirs `shotsTotalRaw` que la
// ligne "Tirs" ci-dessus (un sous-ensemble du même volume de jeu, jamais une valeur
// indépendante), pour que les deux lignes restent cohérentes entre elles pour CE match
// précis.
const SHOTS_ON_TARGET_SHARE = 0.35;
...
function estimateMatchStats(lambdaHome, lambdaAway) {
  ...
  // Tirs cadrés : toujours un sous-ensemble des tirs totaux du même match (jamais
  // recalculé indépendamment) — voir SHOTS_ON_TARGET_SHARE.
  const shotsOnTargetTotalRaw = Math.max(2, shotsTotalRaw * SHOTS_ON_TARGET_SHARE);
  const shotsOnTargetTotal = Math.round(shotsOnTargetTotalRaw);
  ...
  const shotsOnTargetHome = Math.round(shotsOnTargetTotal * homeShare);
  const shotsOnTargetAway = shotsOnTargetTotal - shotsOnTargetHome;
  ...
  return {
    ...
    shotsOnTarget: { home: shotsOnTargetHome, away: shotsOnTargetAway, total: shotsOnTargetTotal, range: rangeFromShare(shotsOnTargetTotal) },
    cards: {
    ...
    raw: {
      ...
      shotsOnTargetTotal: shotsOnTargetTotalRaw,
...
// match, total de chaque équipe séparément (jamais mélangés), tirs, tirs cadrés,
// cartons jaunes, cartons rouges — chacune dérivée des vraies stats de CE match (buts
// attendus par équipe, intensité offensive), jamais une valeur fixe. Le Total (buts)
// accepte une marge de deux lignes quand l'issue est trop incertaine pour une seule
// (voir overUnderLine). Cartons jaunes/rouges affichent, eux, toujours DEUX lignes
// (sûre + risquée, voir riskLines) plutôt qu'une marge conditionnelle — tirs et tirs
// cadrés restent sur une seule ligne chacun, ce marché n'étant pas demandé en deux
// niveaux. Les corners ont leur propre bloc dédié (Total match + mi-temps, voir
// buildMatchStats/components/LiveStatBlock.js), plus aucune ligne ici. Comme le reste
// des pronostics, ces lignes sont calculées UNE SEULE FOIS avant le match et ne sont
// jamais recalculées pendant sa durée (voir pages/api/analyze.js et
// lib/pronosticHistory.js).
function buildMarkets({ totalHomeGoals, totalAwayGoals, extraStats }) {
  return {
    ...
    shotsOnTarget: overUnderLine(extraStats.raw.shotsOnTargetTotal),
...
const STATS_NOTE =
  "Tirs, tirs cadrés, cartons et possession ne sont pas fournis par l'API (plan gratuit) : ce sont des estimations statistiques basées sur l'intensité offensive attendue de chaque équipe, pas une mesure réelle du match. Pour les cartons, deux lignes sont proposées : une option sûre (probabilité réelle élevée) et une option plus risquée (ligne plus poussée, moins certaine, ou pari inverse pour un évènement aussi rare que le carton rouge).";

// Part du total estimé qui survient statistiquement en 1ère mi-temps, en football
// professionnel — même répartition pour les 4 métriques (corners/hors-jeu/fautes/
// touches), faute de données assez fines pour calibrer un partage différent par
// métrique.
//
// IMPORTANT — pronostics figés (correction demandée après coup) : toutes les lignes de
// ce bloc (Total match, Total 1, Total 2, ligne mi-temps) sont calculées UNE SEULE FOIS
// à partir des vraies statistiques des deux équipes, jamais recalculées pendant le
// match. Elles servent de référence stable au parieur — voir pages/api/analyze.js, qui
// calcule ce pronostic une seule fois (computePronostic, jamais à partir du score ou du
// temps de jeu en direct) et le fige ensuite (lib/pronosticHistory.js). Une version
// précédente recalculait ces lignes en direct (rythme réel observé via API-Football,
// bascule automatique 1ère/2ème mi-temps) : ce comportement a été retiré à la demande
// explicite de l'utilisateur, qui a besoin de lignes stables pour parier dessus. La
// ligne "mi-temps" reste donc toujours celle de la 1ère période, même après la pause.
const FIRST_HALF_SHARE = 0.46;

// Construit UN bloc de statistiques (corners, hors-jeu, fautes ou touches) : total du
// match, total par équipe, et une ligne "1ère mi-temps" — toutes calculées une seule
// fois à partir de l'estimation pré-match (extraStats.raw), jamais recalculées.
// `homeShare` est la part du total qui revient à l'équipe à domicile (peut être
// `awayShare` selon la métrique, voir buildMatchStats).
function buildStatBlock({ preMatchTotal, homeShare }) {
  const awayShare = 1 - homeShare;
  const home = preMatchTotal * homeShare;
  const away = preMatchTotal * awayShare;
  const firstHalf = preMatchTotal * FIRST_HALF_SHARE;

  return {
    total: overUnderLine(preMatchTotal),
    home: overUnderLine(home),
    away: overUnderLine(away),
    half: { label: "1ère mi-temps", market: overUnderLine(firstHalf) },
  };
}
...
// à partir des vraies stats de CE match (extraStats.raw) — figées, jamais recalculées
// pendant le match (voir le commentaire sur FIRST_HALF_SHARE ci-dessus).
export function buildMatchStats({ extraStats }) {
  const { homeShare, awayShare } = extraStats.raw;

  return {
    corners: buildStatBlock({ preMatchTotal: extraStats.raw.cornersTotal, homeShare }),
    offsides: buildStatBlock({ preMatchTotal: extraStats.raw.offsidesTotal, homeShare }),
    ...
    fouls: buildStatBlock({ preMatchTotal: extraStats.raw.foulsTotal, homeShare: awayShare }),
    throwIns: buildStatBlock({ preMatchTotal: extraStats.raw.throwInsTotal, homeShare }),
  };
}

const LIVE_STAT_NOTE =
  "Corners, hors-jeu, fautes et touches ne sont pas fournis par l'API (plan gratuit) : ce sont des estimations statistiques basées sur l'intensité offensive et l'équilibre des forces de CE match précis, pas une mesure réelle. Calculées une seule fois avant le match, elles restent identiques pendant toute sa durée (référence stable pour parier), y compris la ligne \"1ère mi-temps\".";

// Justification courte d'une sélection "Issue du match" (voir PROMPT bloc 4 :
// "sous chaque pronostic, afficher une courte raison basée sur les stats") — appuyée
// sur le contexte RÉEL de l'équipe favorite (classement/forme récente), jamais un
// texte générique recopié sur tous les matchs.
function teamContextPhrase(row, teamName) {
  if (row?.position) {
    return `${teamName} pointe à la ${row.position}e place${row.points != null ? ` (${row.points} pts)` : ""}${row.form ? `, forme récente ${row.form}` : ""}`;
  }
  return `${teamName} (classement indisponible pour ce match, estimation basée sur une moyenne)`;
}

function winnerReason(key, home, away, hName, aName) {
  if (key === "draw") {
    return `Issue très incertaine entre ${hName} et ${aName}, aucune équipe nettement favorite selon le modèle statistique.`;
  }
  const favorite = key === "home" ? home : away;
  const favoriteName = key === "home" ? hName : aName;
  return `${teamContextPhrase(favorite, favoriteName)}, favori(te) selon le modèle statistique (buts marqués/encaissés récents des deux équipes).`;
}

// Nombre de buts attendu (round2, 2 décimales) mis en forme comme le reste du site
// (virgule française) — jamais un point, qui ressemblerait à une cote (1.85, 2.40...).
export function formatGoals(x) {
  return String(x).replace(".", ",");
}

export function totalReason(goals, hName, aName) {
  return `Estimation de ${formatGoals(goals.expectedTotal)} but(s) au total pour ce match (${hName} ${formatGoals(goals.expectedHome)} - ${formatGoals(goals.expectedAway)} ${aName}), d'après le rythme offensif des deux équipes.`;
}

export function totalTeamReason(teamName, expected, opponentName) {
  return `${teamName} est attendu(e) autour de ${formatGoals(expected)} but(s) sur ce match, d'après son profil offensif et la défense de ${opponentName}.`;
}

// Justification d'une ligne à répartition domicile/extérieur (corners, tirs, hors-jeu,
// fautes, touches, cartons jaunes) : quel côté concentre la majorité de l'estimation,
// et son vrai décompte — ex. "équipe A concède beaucoup de corners à domicile" (voir
// PROMPT), reformulé ici à partir des VRAIS chiffres calculés pour CE match (jamais un
// texte inventé).
export function splitReason(labelPlural, stat, hName, aName) {
  const dominantIsHome = stat.home >= stat.away;
  const dominantName = dominantIsHome ? hName : aName;
  const dominantCount = dominantIsHome ? stat.home : stat.away;
  const otherCount = dominantIsHome ? stat.away : stat.home;
  return `Estimation d'environ ${stat.total} ${labelPlural} pour ce match, avec une part plus importante attendue du côté de ${dominantName} (${dominantCount} contre ${otherCount}), d'après l'intensité offensive et l'équilibre des forces entre les deux équipes.`;
}

export function redCardReason(redCardExpected) {
  const value = Number.isFinite(redCardExpected) ? redCardExpected : 0;
  return `Estimation de ${formatGoals(round2(value))} carton(s) rouge(s) pour ce match : un évènement qui reste rare en football professionnel, quelle que soit l'équipe.`;
}

// Regroupe, pour CE match précis, toutes les sélections "façon paris sportifs" déjà
// calculées ailleurs dans ce pronostic (1X2, totaux de buts, tirs, cartons, corners,
// hors-jeu, fautes, touches), chacune avec un niveau de confiance RÉEL (même modèle de
// Poisson que le reste du pronostic — jamais une valeur inventée), une courte
// justification textuelle (voir PROMPT bloc 4) et une métadonnée `verify` — comment
// vérifier plus tard, une fois le match terminé, si CETTE sélection précise s'est
// réalisée (voir lib/comboHistory.js) : `{type:"winner", key}` pour le 1X2, ou
// `{type:"line", statKey, line, side}` pour une ligne Plus/Moins, comparée au même
// titre que lib/pronosticVerification.js compare déjà chaque ligne d'un pronostic
// simple. Sert de pool de candidats à "Combiné Vision" (voir lib/combinedVision.js),
// qui n'a plus qu'à choisir la sélection la plus fiable de CE match, sans recalculer
// aucune statistique lui-même.
//
// Pour les marchés qui n'ont normalement qu'une seule ligne "au plus proche" de
// l'estimation (tirs, tirs cadrés, corners, hors-jeu, fautes, touches — voir
// overUnderLine/buildStatBlock, utilisés ailleurs pour l'affichage détaillé du match),
// cette ligne la plus proche est presque toujours un quasi pile-ou-face (~50-55 %) :
// un combiné a besoin d'une vraie marge statistique nette, donc ce pool utilise ici la
// même mécanique de ligne "sûre" que les cartons (riskLines) plutôt que la ligne "au
// plus proche" — dérivée de la MÊME estimation brute déjà calculée (extraStats.raw),
// jamais une deuxième donnée indépendante.
export function buildSelectionCandidates({ probabilities, homeTeamName, awayTeamName, markets, extraStats, home, away, goals }) {
  ...
  const winnerOptions = [
    ...
    { key: "draw", pickLabel: "Match nul", confidence: probabilities.draw },
  ...
  candidates.push({
    ...
    reason: winnerReason(bestWinner.key, home, away, hName, aName),
    verify: { type: "winner", key: bestWinner.key },
  });

  const pushLine = (marketLabel, line, statKey, reason) => {
    candidates.push({
      marketLabel,
  ...
  pushLine("Total", markets.totalGoals.lines[0], "totalGoals", totalReason(goals, hName, aName));
  pushLine("Total 1", markets.totalHome.lines[0], "totalHome", totalTeamReason(hName, goals.expectedHome, aName));
  pushLine("Total 2", markets.totalAway.lines[0], "totalAway", totalTeamReason(aName, goals.expectedAway, hName));
  pushLine("Cartons jaunes", markets.yellowCards.safe, "yellowCards", splitReason("cartons jaunes", extraStats.cards.yellow, hName, aName));
  pushLine("Cartons rouges", markets.redCards.safe, "redCards", redCardReason(extraStats.raw.redCardExpected));

  // Chaque ligne ci-dessous suppose un total réellement chiffré : un total absent
  // (`null`, jamais fourni par aucune source — ex. "touches" avec le profil d'équipe
  // du Bloc 2, voir lib/pronosticFromProfiles.js) ne doit jamais produire de ligne —
  // riskLines(null) traiterait silencieusement l'absence de donnée comme 0, ce qui
  // fabriquerait une fausse ligne "Moins de 0,5" à confiance artificiellement élevée.
  const raw = extraStats.raw;
  const pushRiskLine = (marketLabel, total, statKey, reasonBuilder) => {
    if (total == null || !Number.isFinite(total)) return;
    pushLine(marketLabel, riskLines(total).safe, statKey, reasonBuilder());
  };
  pushRiskLine("Tirs", raw.shotsTotal, "shots", () => splitReason("tirs", extraStats.shots, hName, aName));
  pushRiskLine("Tirs cadrés", raw.shotsOnTargetTotal, "shotsOnTarget", () => splitReason("tirs cadrés", extraStats.shotsOnTarget, hName, aName));
  pushRiskLine("Corners", raw.cornersTotal, "corners", () => splitReason("corners", extraStats.corners, hName, aName));
  pushRiskLine("Hors-jeu", raw.offsidesTotal, "offsides", () => splitReason("hors-jeu", extraStats.offsides, hName, aName));
  pushRiskLine("Fautes", raw.foulsTotal, "fouls", () => splitReason("fautes", extraStats.fouls, hName, aName));
  pushRiskLine("Touches", raw.throwInsTotal, "throwIns", () => splitReason("touches", extraStats.throwIns, hName, aName));
...
export function buildOutcome({ matrix, offsetHome = 0, offsetAway = 0 }) {
...
export function computePronostic({ homeRow, awayRow, homeTeamName, awayTeamName, homeSource = "classement", awaySource = "classement", h2h = null }) {
  ...
  const markets = buildMarkets({ totalHomeGoals: lambdaHome, totalAwayGoals: lambdaAway, extraStats });
  const goals = {
  ...
  return {
    ...
    // Pool de sélections "assez sûres" pour Combiné Vision (voir
    // buildSelectionCandidates plus haut et lib/combinedVision.js) — dérivé des mêmes
    // marchés ci-dessus, jamais recalculé indépendamment.
    selectionCandidates: buildSelectionCandidates({
      probabilities: outcome.probabilities, homeTeamName, awayTeamName, markets, extraStats, home, away, goals,
    }),
...
// Durée conventionnelle d'un match pour la projection du temps restant (voir
// computeLiveOutcome ci-dessous) — les prolongations éventuelles ne sont pas prises
// en compte séparément, leur poids reste marginal sur la projection globale.
const MATCH_MINUTES = 90;

// RETOUR EN ARRIÈRE PARTIEL (demande explicite de l'utilisateur, après le gel complet
// ci-dessus) : SEULS le bloc "Probabilités de victoire" (probabilities), les "Scores
// exacts" (correctScores) et les Totaux de buts (markets.totalGoals/totalHome/
// totalAway, et goals) suivent réellement l'évolution du match — recalculés à partir
// du score et de la minute en direct à chaque actualisation. Tout le reste
// (extraStats, matchStats — Corners/Hors-jeu/Fautes/Touches —, les autres marchés —
// tirs, cartons —, buteurs probables, joueurs susceptibles de prendre un carton, note,
// h2hUsed) reste FIGÉ sur l'estimation pré-match (voir pages/api/analyze.js, qui
// applique ce recalcul par-dessus le pronostic figé, sans jamais le persister —
// chaque actualisation repart des mêmes lambdas pré-match, jamais d'un état déjà
// recalculé la fois précédente).
//
// Repart des buts attendus PRÉ-MATCH (lambdaHome/lambdaAway, jamais recalculés eux-
// mêmes) et ne projette plus que le temps de jeu restant, combiné au score réel actuel.
export function computeLiveOutcome({ lambdaHome, lambdaAway, currentHome, currentAway, minute }) {
  ...
  // Les totaux de buts par équipe doivent refléter la projection sur le match ENTIER
  // (déjà marqués + temps restant), pas seulement les buts encore à venir — sinon
  // "Total 1"/"Total 2" ignoreraient les buts déjà inscrits au tableau d'affichage.
  ...
  return {
    ...
    goals: {
      ...
      range: rangeFromVariance(finalHomeGoals + finalAwayGoals, lambdaHomeRemaining + lambdaAwayRemaining),
      over25: outcome.over25,
      under25: outcome.under25,
      bttsYes: outcome.bttsYes,
      bttsNo: outcome.bttsNo,
    ...
    markets: {
      totalGoals: overUnderLine(finalHomeGoals + finalAwayGoals, { withMargin: true }),
      totalHome: overUnderLine(finalHomeGoals, { withMargin: true }),
      totalAway: overUnderLine(finalAwayGoals, { withMargin: true }),
    },
  };
}
