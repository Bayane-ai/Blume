...
function overUnderLine(expected, { minLine = 0.5, withMargin = false } = {}) {
...
// Seuils de confiance réels (loi de Poisson, pas une approximation) pour le couple de
// lignes "sûre"/"risquée" du bloc "Corners et cartons" (voir components/
// CardsAndCorners.js) : l'option sûre reste très probable (>= SAFE_LINE_CONFIDENCE),
// l'option risquée n'exige plus qu'une confiance modérée (>= RISKY_LINE_CONFIDENCE),
// donc une ligne plus poussée dans le même sens — jamais un coup de dé, toujours
// dérivée de la même distribution que la ligne sûre.
const SAFE_LINE_CONFIDENCE = 0.82;
const RISKY_LINE_CONFIDENCE = 0.55;
// Marge de départ (côté "Moins") au-delà de laquelle la probabilité réelle est déjà
// quasi nulle pour n'importe quelle estimation réaliste (corners/cartons, toujours
// < 25) — juste un point de départ "certainement sûr", pas une borne de recherche.
const RISK_LINE_SEARCH_BUFFER = 30;
// Nombre de pas max pour redescendre jusqu'à `minLine` : doit rester strictement
// supérieur à la distance entre le point de départ et minLine, sinon la recherche
// s'arrête prématurément avant d'avoir trouvé la vraie ligne (voir régression captée
// par capture d'écran : la ligne "sûre" s'arrêtait un cran trop tôt).
const RISK_LINE_MAX_STEPS = 1000;

function lineConfidence(line, value, side) {
  const pOver = probabilityOverLine(line, value);
  return side === "Plus" ? pOver : 1 - pOver;
}

// Ligne X,5 la plus "poussée" dans le sens `side` (la plus grande côté "Plus", la plus
// petite côté "Moins") pour laquelle la probabilité réelle reste encore >= minConfidence
// — la même mécanique sert aussi bien à la ligne sûre (seuil haut) qu'à la ligne
// risquée (seuil plus bas, donc ligne plus poussée), à partir de la MÊME distribution.
function extremeLineAtConfidence(value, side, minConfidence, minLine) {
  if (side === "Plus") {
    let line = minLine;
    for (let i = 0; i < RISK_LINE_MAX_STEPS; i++) {
      const next = line + 1;
      if (lineConfidence(next, value, side) < minConfidence) break;
      line = next;
    }
    return line;
  }
  let line = Math.max(minLine, Math.ceil(value) + 0.5) + RISK_LINE_SEARCH_BUFFER;
  for (let i = 0; i < RISK_LINE_MAX_STEPS; i++) {
    const next = line - 1;
    if (next < minLine || lineConfidence(next, value, side) < minConfidence) break;
    line = next;
  }
  return line;
}
...
function riskLines(expected, { minLine = 0.5 } = {}) {
  ...
  const safeLine = extremeLineAtConfidence(value, side, SAFE_LINE_CONFIDENCE, minLine);
  let riskyLine = extremeLineAtConfidence(value, side, RISKY_LINE_CONFIDENCE, minLine);
  let riskySide = side;
...
// match, total de chaque équipe séparément (jamais mélangés), tirs, corners, cartons
// jaunes, cartons rouges — chacune dérivée des vraies stats de CE match (buts attendus
// par équipe, intensité offensive), jamais une valeur fixe. Le Total (buts) accepte une
// marge de deux lignes quand l'issue est trop incertaine pour une seule (voir
// overUnderLine). Corners/cartons jaunes/cartons rouges affichent, eux, toujours DEUX
// lignes (sûre + risquée, voir riskLines) plutôt qu'une marge conditionnelle — les tirs
// restent sur une seule ligne, ce marché n'étant pas demandé en deux niveaux.
function buildMarkets({ totalHomeGoals, totalAwayGoals, extraStats }) {
  return {
    ...
    corners: riskLines(extraStats.raw.cornersTotal),
...
const STATS_NOTE =
  "Corners, tirs, cartons et possession ne sont pas fournis par l'API (plan gratuit) : ce sont des estimations statistiques basées sur l'intensité offensive attendue de chaque équipe, pas une mesure réelle du match. Pour corners et cartons, deux lignes sont proposées : une option sûre (probabilité réelle élevée) et une option plus risquée (ligne plus poussée, moins certaine, ou pari inverse pour un évènement aussi rare que le carton rouge).";
