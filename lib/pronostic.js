...
function estimateMatchStats(lambdaHome, lambdaAway) {
  ...
  const cornersTotal = Math.max(4, Math.round(AVG_CORNERS_TOTAL * factor));
  const shotsTotal = Math.max(6, Math.round(AVG_SHOTS_TOTAL * factor));
  const cardsTotal = Math.max(1, Math.round(AVG_CARDS_TOTAL * factor));
  ...
  return {
    ...
    goals: {
      ...
      range: rangeFromVariance(
        offsetHome + offsetAway + lambdaHomeRemaining + lambdaAwayRemaining,
