...
// Poisson que le reste du pronostic — jamais une valeur inventée). Sert de pool de
// candidats à "Combiné Vision" (voir lib/combinedVision.js), qui n'a plus qu'à choisir
// la sélection la plus fiable de CE match, sans recalculer aucune statistique lui-même.
//
...
export function buildSelectionCandidates({ probabilities, homeTeamName, awayTeamName, markets, extraStats }) {
  const candidates = [];

  const winnerOptions = [
    { pickLabel: `Victoire ${homeTeamName || "Domicile"}`, confidence: probabilities.home },
    { pickLabel: "Match nul", confidence: probabilities.draw },
    { pickLabel: `Victoire ${awayTeamName || "Extérieur"}`, confidence: probabilities.away },
  ];
  const bestWinner = winnerOptions.reduce((a, b) => (b.confidence > a.confidence ? b : a));
  candidates.push({ marketLabel: "Issue du match", pickLabel: bestWinner.pickLabel, confidence: bestWinner.confidence });

  const pushLine = (marketLabel, line) => {
    candidates.push({ marketLabel, pickLabel: `${line.side} de ${formatLine(line.line)}`, confidence: line.confidence });
  };

  pushLine("Total", markets.totalGoals.lines[0]);
  pushLine("Total 1", markets.totalHome.lines[0]);
  pushLine("Total 2", markets.totalAway.lines[0]);
  pushLine("Cartons jaunes", markets.yellowCards.safe);
  pushLine("Cartons rouges", markets.redCards.safe);

  const raw = extraStats.raw;
  pushLine("Tirs", riskLines(raw.shotsTotal).safe);
  pushLine("Tirs cadrés", riskLines(raw.shotsOnTargetTotal).safe);
  pushLine("Corners", riskLines(raw.cornersTotal).safe);
  pushLine("Hors-jeu", riskLines(raw.offsidesTotal).safe);
  pushLine("Fautes", riskLines(raw.foulsTotal).safe);
  pushLine("Touches", riskLines(raw.throwInsTotal).safe);
