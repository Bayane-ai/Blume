...
function mapSide(raw, side) {
  ...
  return {
    name: pick(node, "name", "title", "short_name", "shortName", "display_name", "slug") || null,
    logo: pick(node, "logo", "logo_url", "logoUrl", "image", "image_url", "crest", "icon", "flag") || null,
...
export function competitionRank(competition, sport) {
  ...
  const majors = sport === "tennis" ? TENNIS_MAJORS : FOOTBALL_MAJORS;
...
export async function fetchSportScoreMatches(sport, { limit = MAX_LIMIT, fetchImpl = fetch } = {}) {
  const res = await fetchImpl(matchesUrl(sport, limit), { headers: { Accept: "application/json" } });
