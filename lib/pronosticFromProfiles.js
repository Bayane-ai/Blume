...
import {
  ...
  buildSelectionCandidates, round1, round2,
} from "./pronostic";
...
export function computeMatchLinesFromProfiles({ homeProfile, awayProfile, homeTeamName, awayTeamName }) {
  ...
  const lambdaHome = Math.max(0.15, expectedHomeGoals);
  const lambdaAway = Math.max(0.15, expectedAwayGoals);
