...
export async function loadUpcoming(sport, { fetchImpl = fetch, now = Date.now() } = {}) {
  ...
  return {
    ...
    // Ce qui a RÉELLEMENT été interrogé : affiché tel quel sous un écran vide, pour
    // qu'une section vide soit toujours explicable sans ouvrir la console.
    diagnostic: {
      window: { from: new Date(now).toISOString().slice(0, 10), to: new Date(now + HORIZON_DAYS * 24 * 3600 * 1000).toISOString().slice(0, 10) },
      sources: [
        { name: "SportScore", httpStatus: ss.error ? null : 200, received: ss.matches.length, error: ss.error },
        {
          name: BLUME_ROUTES[sport] || "(aucune route maison)",
          httpStatus: blume.diagnostic?.httpStatus ?? (blume.error ? null : 200),
          received: blume.matches.length,
          error: blume.error,
        },
      ],
    },
