...
//   tennis     : SportScore + /api/tennis/matches (Live Tennis API — son plan gratuit
//                n'expose pas de calendrier, cette route répond honnêtement
//                "unsupported" ; SportScore reste alors la seule source du tennis)
//
...
async function loadBlumeSource(sport, fetchImpl) {
  ...
  try {
    ...
    if (payload?.unsupported) return { matches: [], error: null, unsupported: payload.message || true };
...
export async function loadUpcoming(sport, { fetchImpl = fetch, now = Date.now() } = {}) {
  ...
  return {
    ...
    unsupported: blume.unsupported || null,
  };
}
