...
// Multi-sport (bloc 0) : sport actuellement sélectionné (voir lib/sports/registry.js
// pour la liste des sports valides et lib/useSport.js pour la lecture/écriture
// réactive côté composants) — football par défaut, mémorisé comme le reste de ce
// cookie, jamais soumis au consentement (voir le commentaire en tête de fichier).
const VALID_SPORTS = ["football", "basketball", "tennis"];
const DEFAULT_PREFS = { theme: "dark", lastTab: null, favoriteCompetitions: [], sport: "football" };
...
function parseCookieValue(raw) {
  try {
    ...
    return {
      ...
      sport: VALID_SPORTS.includes(parsed?.sport) ? parsed.sport : "football",
    };
  } catch (e) {
