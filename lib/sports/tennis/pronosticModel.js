...
// match tennis, même forme que lib/pronostic.js#buildSelectionCandidates (football) :
// {marketLabel, pickLabel, confidence, reason, verify}. Pas de match nul (verify.type
// "winner" n'a que "home"/"away"). Les breaks sont volontairement ABSENTS de ce pool
// (contrairement au reste du bloc 7) : aucune source connectée à Blume ne fournit un
// décompte réel et fiable du nombre TOTAL de breaks une fois le match terminé (voir
// lib/sports/tennis/pronosticHistory.js, même limitation documentée pour le bloc 8) —
// une sélection de combiné qui ne pourrait jamais être réglée resterait "En cours"
// pour toujours, ce que Combiné Vision doit justement éviter.
function buildTennisSelectionCandidates({ probabilities, homeTeamName, awayTeamName, gameTotals, aces }) {
  ...
  pushLine("Total jeux", gameTotals.total, "totalGames");
  pushLine("Total jeux 1", gameTotals.home, "totalGamesHome");
  pushLine("Total jeux 2", gameTotals.away, "totalGamesAway");
  pushLine("Aces", aces.total, "acesTotal");
  pushLine("Aces 1", aces.home, "acesHome");
  pushLine("Aces 2", aces.away, "acesAway");
...
export function computeTennisPronostic({ homeProfile, awayProfile, homeTeamName, awayTeamName, h2hGames = null, homeId = null, bestOf = 3, surface = null }) {
  ...
  return {
    ...
    // buildTennisSelectionCandidates ci-dessus et lib/combinedVision.js) — dérivé des
    // mêmes marchés que ci-dessus, jamais recalculé indépendamment.
    selectionCandidates: buildTennisSelectionCandidates({ probabilities, homeTeamName: hName, awayTeamName: aName, gameTotals, aces }),
  ...
  if (!liveState) {
    ...
    return {
      ...
      setScores: setScoreDistribution(pSet, bestOf).sort((a, b) => b.probability - a.probability).slice(0, 4).map((d) => ({
        score: d.winner === "p1" ? d.score : d.score.split("-").reverse().join("-"), winner: d.winner, probability: round1(d.probability),
      })),
      gameTotals: {
