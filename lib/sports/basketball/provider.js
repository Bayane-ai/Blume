...
// Timeout explicite de 10 s et DEUX tentatives avec attente croissante. Sans timeout,
// une requête lente pendait jusqu'à l'expiration de la fonction Vercel ; sans reprise,
// une coupure réseau d'une seconde comptait comme une panne franche.
// Un 429 n'est JAMAIS réessayé : le quota est épuisé, insister le creuserait davantage.
const TIMEOUT_MS = 10 * 1000;
const TENTATIVES = 2;

async function fetchAvecReprise(url, key) {
  let derniere = null;
  for (let i = 0; i < TENTATIVES; i += 1) {
    try {
      ...
      if (r.status === 429 || r.ok || i === TENTATIVES - 1) return r;
      derniere = new Error(`HTTP ${r.status}`);
    } catch (e) {
      derniere = e;
      if (i === TENTATIVES - 1) throw e;
    }
    await new Promise((res) => setTimeout(res, 500 * 2 ** i)); // 500 ms puis 1 s
  }
  throw derniere;
}
...
async function basketballFetch(path, key) {
  try {
    ...
    // PAGINATION COMPLÈTE : l'enveloppe API-SPORTS plafonne chaque page (~100 éléments)
    // et annonce le total dans `paging: {current, total}`. Ne lire que la première page
    // — comportement précédent — tronquait silencieusement les journées chargées, et
    // coupait en priorité les petites ligues, classées après les grandes dans l'ordre
    // interne de l'API. On boucle donc sur toutes les pages annoncées (même correctif
    // que lib/apiFootball.js).
    const sep = path.includes("?") ? "&" : "?";
    const collected = [];
    let page = 1;
    let totalPages = 1;
    while (page <= totalPages) {
      const r = await fetchAvecReprise(`${BASE}${path}${sep}page=${page}`, key);
      ...
      if (r.status === 429) {
        ...
        // Les pages déjà obtenues restent valables : mieux qu'un résultat partiel réel
        // que rien du tout (jamais de donnée inventée pour combler le reste).
        if (collected.length > 0) break;
      }
      if (!r.ok) {
        if (collected.length > 0) break;
        throw new Error(`API-Basketball a répondu ${r.status} sur ${path}`);
      }
      ...
      if (hasErrors) {
        if (collected.length > 0) break;
        throw new Error(`Erreur API-Basketball : ${JSON.stringify(errors)}`);
      }
    ...
    // Persisté (voir lib/apiQuota.js#getLastError, affiché sur /admin) pour diagnostiquer
    // à distance (abonnement manquant, panne, quota) sans avoir besoin des logs Vercel.
...
// TTL par journée, calibré sur le quota RÉEL du plan gratuit (100 appels/jour).
// L'ancien réglage — 5 min pour les 8 journées — consommait 2 304 appels/jour, soit 24
// fois le quota : il était épuisé en moins de 7 minutes de trafic, et tout le sport
// basculait ensuite en erreur pour le reste de la journée. C'était la cause du bandeau
// rouge.
//
// Le calendrier des jours suivants ne bouge quasiment pas ; seule la journée en cours
// mérite d'être rafraîchie souvent. Budget résultant :
//   aujourd'hui  TTL 30 min -> 48 appels/jour
//   J+1 à J+7    TTL 12 h   -> 14 appels/jour
//   total                      62 appels/jour, sous le quota, avec de la marge.
const GAMES_BY_DATE_TTL_MS = 30 * 60 * 1000;
const GAMES_FUTURE_TTL_MS = 12 * 60 * 60 * 1000;

function ttlPourDate(dateStr) {
  const aujourdHui = new Date().toISOString().slice(0, 10);
  return dateStr === aujourdHui ? GAMES_BY_DATE_TTL_MS : GAMES_FUTURE_TTL_MS;
}
// Un résultat VIDE ne doit jamais être traité comme une vérité durable : sans ça, un
// seul appel raté (clé absente à cet instant, quota épuisé, panne passagère) fige le
// sport à "aucun match" pendant toute la durée du cache, même une fois le problème
// résolu. On le remet donc en question très vite, alors qu'une liste réellement pleine
// peut être conservée normalement.
...
export async function getLiveGames(key) {
  ...
  return liveGamesCache.get("live", async () => {
    ...
    const persistedEmpty = persisted && (!Array.isArray(persisted.payload) || persisted.payload.length === 0);
    const persistedFresh =
      persisted && Date.now() - persisted.fetchedAt < (persistedEmpty ? EMPTY_RESULT_TTL_MS : LIVE_GAMES_TTL_MS);
...
export async function getGamesByDate(dateStr, key) {
  ...
  return gamesByDateCache.get(dateStr, async () => {
    ...
    if (persisted) {
      ...
      const isEmpty = !Array.isArray(persisted.payload) || persisted.payload.length === 0;
      // TTL adaptatif : la journée en cours est rafraîchie souvent, les suivantes très
      // rarement (leur calendrier ne bouge quasiment pas). C'est ce qui ramène la
      // consommation sous le quota gratuit — voir le calcul au-dessus de
      // GAMES_BY_DATE_TTL_MS.
      // Une liste vide n'est resservie qu'une minute : au-delà, on retente réellement.
      if (age < (isEmpty ? EMPTY_RESULT_TTL_MS : ttlPourDate(dateStr))) return persisted.payload;
    }

    try {
      // `timezone=UTC` explicite : dateStr est calculé en UTC (voir pages/api/
      // basketball/matches.js), même raisonnement que lib/apiFootball.js#getFixturesByDate
      // — sans ce paramètre, l'API pourrait attribuer un match à un jour calendaire
      // différent selon son propre fuseau par défaut, le faisant disparaître de TOUS
      // les jours interrogés.
      const games = await basketballFetch(`/games?date=${dateStr}&timezone=UTC`, key);
      console.log(`[API-Basketball] /games?date=${dateStr} : ${games.length} match(s) reçu(s)`);
      writePersistentCache(cacheKey, games);
      return games;
    } catch (e) {
      // Panne, quota épuisé, clé révoquée : on ressert la dernière liste connue, même
      // périmée, plutôt que de faire disparaître des matchs déjà récupérés. Un écran
      // vide (ou un bandeau rouge) est toujours pire qu'une donnée un peu datée.
      if (persisted) {
        console.warn(
          `[API-Basketball] ${dateStr} : ${e.message} — cache persistant resservi (${Array.isArray(persisted.payload) ? persisted.payload.length : 0} match(s), âge ${Math.round((Date.now() - persisted.fetchedAt) / 60000)} min)`
        );
        return persisted.payload;
      }
      throw e;
