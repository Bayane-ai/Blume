...
// Cache PERSISTANT (table api_football_cache, voir supabase/migrations/
// 0015_api_football_cache.sql) — signalement réel : des compétitions entières
// (Russie, Écosse, Norvège, Supercoupe des Pays-Bas...) restaient invisibles alors que
// API_FOOTBALL_KEY était bien configurée en production. Cause identifiée : le cache en
// mémoire ci-dessous (Map/variable de module) ne survit QUE le temps d'une instance
// Vercel "chaude" — sous trafic réel, chaque instance froide repartant de zéro refait
// TOUS les appels API-Football depuis le début (jusqu'à 9 appels par chargement de
// "Matchs à venir"), ce qui épuise le quota gratuit (100 requêtes/jour) en quelques
// chargements de page ; une fois épuisé, toute compétition qui dépend uniquement
// d'API-Football (donc absente des 12 grandes ligues de lib/competitions.js) disparaît
// silencieusement jusqu'au renouvellement du quota le lendemain. Ces deux fonctions
// laissent une instance froide réutiliser la dernière réponse encore fraîche plutôt
// que de rappeler l'API, et font que la pause anti-quota (429) est respectée par
// TOUTES les instances plutôt que par la seule qui a reçu le 429 — jamais bloquant :
// toute erreur (table absente car migration pas encore exécutée, Supabase
// indisponible...) est avalée ici et traitée exactement comme "pas de cache", pour ne
// jamais casser le comportement déjà en place (cache en mémoire + appel réseau direct).
...
function writePersistentCache(key, payload) {
  ...
  // appel (potentiellement sur une tout autre instance serverless).
  (async () => {
    try {
      const supabase = getSupabaseAdmin();
      await supabase
...
// suivi en direct. football-data.org (voir liveListCache.js, cache de 2,5s) reste la
// source principale et déjà quasi temps réel pour les compétitions qu'il couvre :
// API-Football n'est que la couche supplémentaire pour tout le reste (petites
// fédérations) — 90s de cache pour cette seule couche protège le quota quotidien sans
// rendre ces matchs-là indisponibles.
const LIVE_LIST_TTL_MS = 90000;
...
// Une fois le quota quotidien dépassé (HTTP 429), continuer à retenter toutes les
// 30s/6h comme si de rien n'était ne fait qu'ajouter des appels en échec pour rien
// (aucune chance de succès avant la remise à zéro quotidienne côté API-Football) : on
// marque une pause avant tout nouvel appel réel, en laissant chaque fonction
// (getAllLiveFixtures, getFixturesByDate, ...) retomber sur son repli habituel
// (dernière donnée connue, ou liste vide) exactement comme pour toute autre erreur.
const QUOTA_BACKOFF_MS = 60 * 60 * 1000; // 1h
let quotaBackoffUntil = 0;
const QUOTA_BACKOFF_CACHE_KEY = "quota_backoff_until";

// Le quota (100 requêtes/jour) est partagé par TOUT le compte API-Football, pas par
// instance serverless : une fois épuisé, une instance qui vient de démarrer (mémoire
// vide, `quotaBackoffUntil` à 0) ne le sait pas et retenterait quand même, gaspillant
// un appel de plus au moment même où chaque appel compte le plus. Cette vérification
// consulte donc aussi le cache persistant (voir readPersistentCache ci-dessus) avant
// de conclure "pas de pause en cours".
...
// L'enveloppe API-SPORTS renvoie au plus ~100 résultats par page (champ `paging:
// {current, total}` dans la réponse) — un jour chargé (toutes fédérations, toutes
// divisions, catégories jeunes comprises) dépasse largement ce seuil dans le monde
// entier. Ne lire QUE la page 1, comme avant cette correction, coupait silencieusement
// les fixtures qui tombent après la 100e (souvent les petites fédérations, classées
// après les plus grandes selon l'ordre interne de l'API) — c'est très probablement
// la cause réelle de championnats entiers absents (ex : Russie, Japon) alors qu'ils
// existent bien côté API-Football. On boucle donc sur toutes les pages annoncées.
async function apiFootballFetch(path, key) {
  if (await isQuotaBackoffActive()) {
    throw new Error("API-Football : pause en cours après un dépassement de quota (voir logs)");
  }
  ...
  while (page <= totalPages) {
    const r = await fetch(`${BASE}${path}${sep}page=${page}`, { headers: { "x-apisports-key": key } });
    if (r.status === 429) {
      quotaBackoffUntil = Date.now() + QUOTA_BACKOFF_MS;
      writePersistentCache(QUOTA_BACKOFF_CACHE_KEY, { until: quotaBackoffUntil });
      console.warn("[API-Football] 429 reçu (quota quotidien probablement dépassé) : pause d'environ 1h avant nouvel essai");
      // Les pages déjà récupérées avant le 429 restent valables — mieux qu'un résultat
      // partiel réel que rien du tout (jamais une donnée inventée pour combler le reste).
      if (collected.length > 0) break;
      throw new Error("API-Football a répondu 429");
    }
    if (!r.ok) {
      if (collected.length > 0) break;
      throw new Error(`API-Football a répondu ${r.status}`);
    }
    ...
    if (hasErrors) {
      if (collected.length > 0) break;
      throw new Error(`Erreur API-Football : ${JSON.stringify(errors)}`);
    }
...
export async function getAllLiveFixtures(key) {
  ...
  liveListInFlight = (async () => {
    try {
      // Avant tout appel réseau : une autre instance serverless a peut-être déjà
      // récupéré une liste encore fraîche il y a quelques secondes — la mémoire de ce
      // module n'est PAS partagée entre instances Vercel, donc ce cache persistant
      // (voir readPersistentCache) est ce qui protège réellement le quota quotidien
      // sous trafic réel, contrairement au cache en mémoire seul (liveListCache), qui
      // ne survit qu'au sein d'une même instance restée chaude.
      const persisted = await readPersistentCache("live_all");
      if (persisted && Date.now() - persisted.fetchedAt < LIVE_LIST_TTL_MS) {
        liveListCache = { fixtures: persisted.payload, fetchedAt: persisted.fetchedAt };
        return persisted.payload;
      }
      const fixtures = await apiFootballFetch("/fixtures?live=all", key);
      liveListCache = { fixtures, fetchedAt: Date.now() };
      writePersistentCache("live_all", fixtures);
      return fixtures;
    } catch (e) {
...
export async function getFixturesByDate(dateStr, key) {
  ...
  const promise = (async () => {
    try {
      // Même raisonnement que getAllLiveFixtures ci-dessus : avant tout appel réseau,
      // une autre instance serverless a peut-être déjà récupéré cette date récemment
      // (le cache en mémoire, `upcomingCache`, ne survit pas d'une instance froide à
      // l'autre) — le cache persistant (voir readPersistentCache) est ce qui protège
      // réellement le quota quotidien sous trafic réel, où plusieurs instances
      // coexistent. Clé unique par jour calendaire (un seul appel API-Football
      // partagé par TOUS les visiteurs pour cette date, quel que soit le nombre
      // d'instances Vercel actives).
      const cacheKey = `upcoming:${dateStr}`;
      const persisted = await readPersistentCache(cacheKey);
      if (persisted && Date.now() - persisted.fetchedAt < UPCOMING_TTL_MS) {
        upcomingCache.set(dateStr, { fixtures: persisted.payload, fetchedAt: persisted.fetchedAt });
        return persisted.payload;
      }
      // `timezone=UTC` explicite : dateStr est calculé en UTC (voir pages/api/matches.js) ;
      // sans ce paramètre, API-Football pourrait attribuer un match à un jour calendaire
      // différent selon son propre fuseau par défaut, faisant disparaître ce match de TOUS
      // les jours interrogés (ni la veille ni le lendemain ne le récupèrent alors).
      const fixtures = await apiFootballFetch(`/fixtures?date=${dateStr}&timezone=UTC`, key);
      upcomingCache.set(dateStr, { fixtures, fetchedAt: Date.now() });
      writePersistentCache(cacheKey, fixtures);
      return fixtures;
    } catch (e) {
      if (cached) return cached.fixtures;
      const persisted = await readPersistentCache(`upcoming:${dateStr}`);
      if (persisted) return persisted.payload;
      return [];
    } finally {
...
// TOUTES les compétitions actives cette saison, tous pays confondus (GET /leagues?
// current=true, sans paramètre de pays) — jamais une liste codée en dur à tenir à
// jour manuellement : un nouveau championnat qui démarre une saison (ou une coupe qui
// en change) apparaît ici automatiquement au prochain rafraîchissement du cache,
// aucune modification de code n'est nécessaire. N'est PAS utilisée pour filtrer les
// matchs affichés (voir getFixturesByDate ci-dessus, déjà sans aucun filtre de ligue
// ni de pays) : sert uniquement de repère diagnostique — le nombre de compétitions
// actives est journalisé (logs Vercel) pour pouvoir vérifier après coup qu'aucune
// d'elles n'est durablement absente des matchs réellement affichés.
const ACTIVE_LEAGUES_TTL_MS = 24 * 3600 * 1000; // 24h : la liste des compétitions d'une saison ne change pas d'un jour à l'autre
let activeLeaguesCache = null; // { leagues, fetchedAt }
let activeLeaguesInFlight = null;

export async function getActiveLeagues(key) {
  if (!key) return [];
  if (activeLeaguesCache && Date.now() - activeLeaguesCache.fetchedAt < ACTIVE_LEAGUES_TTL_MS) {
    return activeLeaguesCache.leagues;
  }
  if (activeLeaguesInFlight) return activeLeaguesInFlight;

  activeLeaguesInFlight = (async () => {
    try {
      const leagues = await apiFootballFetch("/leagues?current=true", key);
      activeLeaguesCache = { leagues, fetchedAt: Date.now() };
      console.log(`[API-Football] /leagues?current=true : ${leagues.length} compétition(s) active(s) reçue(s)`);
      return leagues;
    } catch (e) {
      if (activeLeaguesCache) return activeLeaguesCache.leagues;
      return [];
    } finally {
      activeLeaguesInFlight = null;
    }
  })();
  return activeLeaguesInFlight;
}

// Les 10 derniers matchs (toutes compétitions confondues) d'UNE équipe précise, quel
// que soit leur statut (terminé, à venir, reporté...) — filtrer aux matchs réellement
// terminés reste la responsabilité de l'appelant (voir lib/teamStatProfiles.js, "Bloc
// 1" : profil statistique par équipe, construit à partir de ses vrais matchs passés).
// Cache assez long (12h) : la liste des derniers matchs d'une équipe ne change qu'une
// fois par match joué, jamais plusieurs fois par jour — indispensable pour rester
// soutenable, ce calcul de profil appelant déjà /fixtures/statistics une fois par
// match utilisé (voir getFixtureStatistics).
const TEAM_LAST_FIXTURES_TTL_MS = 12 * 3600 * 1000; // 12h
const teamLastFixturesCache = new Map(); // teamId -> { fixtures, fetchedAt }
const teamLastFixturesInFlight = new Map();

export async function getTeamLastFixtures(teamId, key, { last = 10 } = {}) {
  if (!key || !teamId) return [];
  const cacheKey = `${teamId}:${last}`;
  const cached = teamLastFixturesCache.get(cacheKey);
  if (cached && Date.now() - cached.fetchedAt < TEAM_LAST_FIXTURES_TTL_MS) return cached.fixtures;

  const pending = teamLastFixturesInFlight.get(cacheKey);
  if (pending) return pending;

  const promise = (async () => {
    try {
      const fixtures = await apiFootballFetch(`/fixtures?team=${teamId}&last=${last}`, key);
      teamLastFixturesCache.set(cacheKey, { fixtures, fetchedAt: Date.now() });
      return fixtures;
    } catch (e) {
      if (cached) return cached.fixtures;
      return [];
    } finally {
      teamLastFixturesInFlight.delete(cacheKey);
    }
  })();
  teamLastFixturesInFlight.set(cacheKey, promise);
  return promise;
}
...
// Statistiques RÉELLES et FINALES d'un match terminé (corners, hors-jeu, fautes, tirs,
// cartons) — utilisées UNE SEULE FOIS, à la fin du match, pour le compte-rendu
// "Probabilités réussies/échouées" (voir lib/pronosticVerification.js), jamais pendant
// le match (les lignes de pronostics restent figées, voir lib/pronostic.js). Une
// version précédente utilisait cette même source pour recalculer les pronostics EN
// DIRECT : ce comportement a été retiré à la demande explicite de l'utilisateur ; seul
// le compte-rendu de fin de match, une fois le résultat définitivement connu, réutilise
// cette source.
const FIXTURE_STATS_TTL_MS = 5 * 60 * 1000; // 5 min : largement assez pour un compte-rendu ponctuel
...
// { corners, offsides, fouls, shots, shotsOnTarget, yellowCards, redCards }, chacun
// { home, away, total } — `fixtureHomeId` (l'id API-Football, pas celui de football-
// data.org) sert à
// savoir laquelle des deux entrées correspond à l'équipe à domicile. Renvoie `null` si
// la réponse n'a pas la forme attendue (les deux équipes) plutôt qu'un objet à moitié
...
export function mapFixtureStatistics(raw, fixtureHomeId) {
  ...
  const build = (type) => {
    const home = statisticValue(homeEntry?.statistics, type);
    const away = statisticValue(awayEntry?.statistics, type);
    return { home, away, total: home + away };
  };
  // Possession : l'API la renvoie déjà en pourcentage ("55%", "45%") — un total home+away
  // n'a pas de sens ici (toujours ~100), contrairement aux autres statistiques comptées ;
  // seules les deux parts individuelles sont utiles (voir lib/teamStatProfiles.js, note
  // PROMPT 1 "style de jeu : possession moyenne").
  const possessionHome = statisticValue(homeEntry?.statistics, "Ball Possession");
  const possessionAway = statisticValue(awayEntry?.statistics, "Ball Possession");
  const hasPossession = possessionHome > 0 || possessionAway > 0;

  return {
    corners: build("Corner Kicks"),
    offsides: build("Offsides"),
    fouls: build("Fouls"),
    shots: build("Total Shots"),
    shotsOnTarget: build("Shots on Goal"),
    yellowCards: build("Yellow Cards"),
    redCards: build("Red Cards"),
    possession: hasPossession ? { home: possessionHome, away: possessionAway } : null,
  };
}
