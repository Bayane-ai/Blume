...
import { recordQuotaUsage, isQuotaExhausted, recordLastError } from "./apiQuota";

const BASE = "https://v3.football.api-sports.io";
const SPORT_KEY = "football";

// Cache PERSISTANT partagé entre toutes les instances serverless — voir
// lib/apiSportsCache.js pour l'explication complète (signalement réel : des
// compétitions entières comme la Russie, l'Écosse, la Norvège ou la Supercoupe des
// Pays-Bas restaient invisibles alors que API_FOOTBALL_KEY était bien configurée en
// production, à cause du cache en mémoire ci-dessous qui ne survit pas d'une instance
// froide à l'autre).
...
async function apiFootballFetch(path, key) {
  try {
    ...
    // confirme un quota à 0 aujourd'hui — pas besoin d'attendre le prochain 429 (qui
    // gaspillerait un appel de plus, voir lib/apiQuota.js).
    if (await isQuotaExhausted(SPORT_KEY)) {
      throw new Error("API-Football : quota quotidien épuisé (confirmé par l'API, voir page admin)");
    }
    ...
    while (page <= totalPages) {
      ...
      if (!r.ok) {
        ...
        throw new Error(`API-Football a répondu ${r.status} sur ${path} : ${body.slice(0, 300)}`);
      }
    ...
    // silencieusement transformée en liste vide"), même si l'appelant retombe ensuite
    // sur un cache/repli gracieux — affiché sur /admin (lib/apiQuota.js#getLastError).
    console.error(`[API-Football] ${e.message}`);
...
export function mapFixtureToLiveMatch(fixture) {
  ...
  return {
    ...
    competition: {
      code: fixture?.league?.id != null ? `af-${fixture.league.id}` : "",
      name: fixture?.league?.name || "Compétition",
      // Pays de la compétition (voir mapFixtureToUpcomingMatch, même champ, déjà utilisé
      // côté "matchs à venir") : nécessaire ici aussi pour que les sections filtrées par
      // pays (voir lib/featuredCompetitions.js — championnats russe/suédois/slovaque/
      // letton) fonctionnent identiquement en direct et à venir, jamais seulement l'un
      // des deux.
      area: fixture?.league?.country || "",
      emblem: fixture?.league?.logo || "",
    },
