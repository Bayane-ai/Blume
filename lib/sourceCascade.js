...
export async function runCascade(sources) {
  ...
  // Une tentative est décrite DEUX fois, sous les deux vocabulaires du projet :
  //  - { nom, statut, httpCode, recus, erreur } : la forme exigée par le bloc 2, celle
  //    que renvoient les routes et que lit le contrôle de santé ;
  //  - { name, httpStatus, received, error } : la forme historique, encore lue par
  //    lib/upcomingMatches.js et l'affichage.
  // Les deux décrivent le même fait : aucune conversion ne peut les faire diverger.
  const tentative = ({ nom, httpCode, recus, erreur, ...reste }) => ({
    nom,
    statut: erreur ? (reste.skipped ? "non configurée" : "échec") : recus > 0 ? "ok" : "vide",
  ...
  for (const source of sources) {
    if (source?.skip) {
      ...
      attempts.push(
        tentative({ nom: source.nom || source.name, httpCode: null, recus: 0, erreur: source.skip, skipped: true })
      );
      continue;
    }
    ...
    try {
      ...
      const list = Array.isArray(out) ? out : out?.matches || out?.matchs || [];
      attempts.push(
        tentative({
          nom: source.nom || source.name,
          httpCode: Array.isArray(out) ? 200 : out?.httpCode ?? out?.httpStatus ?? 200,
          recus: list.length,
          erreur: Array.isArray(out) ? null : out?.error || null,
      ...
      attempts.push(
        tentative({
          nom: source.nom || source.name,
          httpCode: e?.httpCode ?? e?.httpStatus ?? null,
          recus: 0,
