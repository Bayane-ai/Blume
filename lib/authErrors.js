// Traduit les erreurs Supabase Auth en messages clairs, en français — connexion
// UNIQUEMENT par email (voir pages/connexion.js) : code à 6 chiffres reçu par email
// (Google a été complètement abandonné, ne pas réintroduire). Un seul écran, une seule
// fonction d'erreur : il n'y a plus de distinction inscription/connexion côté Supabase
// non plus (signInWithOtp crée le compte automatiquement s'il n'existe pas encore).
...
export function friendlyAuthError(error) {
  ...
  if (code === "otp_disabled" || code === "email_provider_disabled" || code === "signup_disabled") {
    return "La connexion par email est momentanément indisponible. Réessaie dans quelques instants.";
  }
  // "missing_config" (voir lib/supabaseClient.js) : NEXT_PUBLIC_SUPABASE_URL et/ou
  // NEXT_PUBLIC_SUPABASE_ANON_KEY sont vides dans CE déploiement — le client ne tente
  // même pas de contacter Supabase (aucune requête réseau n'a lieu). Le message est
  // ici affiché TEL QUEL (jamais un texte vague) : il dit exactement quelle variable
  // manque et rappelle qu'un redéploiement est nécessaire après l'avoir ajoutée sur
  // Vercel — ce n'est jamais une donnée sensible (aucune clé/secret n'y figure), donc
  // rien n'empêche de le montrer directement à l'écran pour le diagnostic.
  if (code === "missing_config") {
    return error.message;
  }
  // "Invalid path specified" : les variables existent mais Supabase refuse la requête
  // (ex. barre oblique finale ou espace resté dans l'URL malgré la normalisation, ou
  // une URL qui ne correspond à aucun projet Supabase réel) — un cas différent du
  // précédent (ici, une vraie requête a bien été envoyée). Le détail technique brut
  // est conservé dans le message pour le diagnostic.
  if (msg.includes("invalid path specified")) {
    return `Erreur de configuration du service de connexion (URL Supabase invalide). Détail technique : ${error.message}`;
  }
